...
# Print out the messages for testing
...
# Save message in in-memory table
# Use SQL Query to read data
#.format('memory') defines how spark process data.
...
# Spark automatcially convert data format to binary
...
# Format the data to two-columns dataframe
def prepare_df_to_kafka_sink(df, value_columns, key_column=None):
    columns = df.columns
    # Concat all values into a single string
